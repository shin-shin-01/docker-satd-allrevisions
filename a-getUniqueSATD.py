...
def get_unique_satd_toCSV(df):
    df[["Comments"]].drop_duplicates().to_csv("./a-extractComment/uniqueALLSATD.csv")
...
def get_unique_satd_with_deleteInfo_toCSV(df):
    ...
    tmp = tmp[["Comments"]].drop_duplicates().to_csv("./a-extractComment/SATDnotDeleted.csv")

    tmp = df[df["コメント追加からコメント削除までの日数"] == "ファイル削除"]
    tmp.to_csv("./a-extractComment/SATDfileDeletedWithInfo.csv")
    tmp = tmp[["Comments"]].drop_duplicates().to_csv("./a-extractComment/SATDfileDeleted.csv")
    ...
    tmp = tmp[["Comments"]].drop_duplicates().to_csv("./a-extractComment/SATDDeleted.csv")
